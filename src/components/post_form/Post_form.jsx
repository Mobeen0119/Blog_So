...
export default function PostForm({ post }) {
    ...
    const [imagePreview, setImagePreview] = useState(
        (post?.images || post?.image) ? service.getfilePreview(post?.images || post?.image) : null
    );
    ...
    const submit = async (data) => {
        ...
        try {
            ...
            let imageId = post?.images || post?.image || undefined;
            ...
            if (data.images && data.images[0]) {
                ...
                if (file) {
                    ...
                    if (post?.images || post?.image) {
                        await service.deletefile(post?.images || post?.image);
                    }
                }
            }
        ...
        <div className="max-w-4xl mx-auto p-6">
            ...
            <form onSubmit={handleSubmit(submit)} className="space-y-6 bg-white p-6 rounded-lg shadow-md">
                ...
                <div>
                    ...
                    {(imagePreview || post?.images || post?.image) && (
                        <div className="mt-4">
                            <p className="text-sm text-gray-600 mb-2">Preview:</p>
                            <img
                                src={imagePreview || service.getfilePreview(post?.images || post?.image)}
                                alt="Preview"
